...
UDPClient::UDPClient(int domain, int service, int protocol, int port, uint32_t interface) {
...
void UDPClient::send_udp_message(const std::string &msg) {
    sendto(udp_socket->get_sock(), msg.c_str(), msg.length(), 0,
           (struct sockaddr*) &server_address, sizeof(server_address));
    std::cout << "[UDP Sent]: " << msg << std::endl;
}

ConnectingSocket* UDPClient::get_udp_socket() const {
    return udp_socket;
}