...
TTU_ChatBase::TTU_ChatBase(const std::string &filen_crt, const std::string &filen_key, bool server) : 
                                     TTU(filen_crt, filen_key, server), is_server(server) {
    tls_context = create_context(is_server);
    dtls_context = create_context(is_server);
}

TTU_ChatBase::~TTU_ChatBase() {
    SSL_CTX_free(tls_context);
    SSL_CTX_free(dtls_context);
}

SSL_CTX* TTU_ChatBase::get_tls_context() const {
    return tls_context;
}

SSL_CTX* TTU_ChatBase::get_dtls_context() const {
    return dtls_context;
}

void TTU_ChatBase::handle_tls_chat(SSL* ssl, bool is_server) {
...
// multiple clients
void TTU_ChatBase::handle_dtls_chat(SSL_CTX* dtls_ctx, int udp_sock, struct sockaddr_in &peer_addr, bool is_server) {
    char buffer[BUFFER_SIZE];
